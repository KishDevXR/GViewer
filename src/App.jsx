...
export default function App() {
  ...
  const handleShareToFirebase = async () => {
    ...
    try {
      ...
      if (!modelFile || !modelFile.size) {
        console.error("Model file is missing or empty:", modelFile);
        alert("Model file is missing or empty. Please select a valid file.");
        setLoadingUpload(false);
        return;
      }
      console.log("Uploading model to Firebase:", {
        name: modelFile.name,
        size: modelFile.size,
        type: modelFile.type
      });
      ...
      try {
        ...
        console.error("Firebase uploadBytes error (variant1.glb):", uploadErr);
        alert("Failed to upload model to Firebase Storage: " + uploadErr.message);
        setLoadingUpload(false);
        return;
      }
      let modelDownloadUrl = null;
      try {
        modelDownloadUrl = await getDownloadURL(modelRef);
      } catch (urlErr) {
        console.error("Firebase getDownloadURL error (variant1.glb):", urlErr);
        alert("Failed to get download URL for uploaded model: " + urlErr.message);
        setLoadingUpload(false);
        return;
      }
      ...
      for (let i = 0; i < variants.length; i++) {
        ...
        try {
          ...
          console.error(`Firebase uploadBytes error (variant${i + 1}.glb):`, uploadErr);
          alert(`Failed to upload variant ${i + 1} to Firebase Storage: ` + uploadErr.message);
          setLoadingUpload(false);
          return;
        }
        let variantDownloadUrl = null;
        try {
          variantDownloadUrl = await getDownloadURL(variantRef);
        } catch (urlErr) {
          console.error(`Firebase getDownloadURL error (variant${i + 1}.glb):`, urlErr);
          alert(`Failed to get download URL for variant ${i + 1}: ` + urlErr.message);
          setLoadingUpload(false);
          return;
        }
        variantUrls.push(variantDownloadUrl);
      ...
      // Upload preview images and USDZ files
      for (let i = 0; i < previewImages.length; i++) {
        ...
        try {
          ...
          console.error(`Firebase uploadBytes error (preview variant ${variantName}.glb):`, uploadErr);
          alert(`Failed to upload preview variant ${variantName} to Firebase Storage: ` + uploadErr.message);
          setLoadingUpload(false);
          return;
        }
        let variantDownloadUrl = null;
        try {
          ...
          console.error(`Firebase getDownloadURL error (preview variant ${variantName}.glb):`, urlErr);
          alert(`Failed to get download URL for preview variant ${variantName}: ` + urlErr.message);
          setLoadingUpload(false);
          return;
        }
        variantUrls.push(variantDownloadUrl);
        ...
        if (usdzFiles[i + 1]) {
          ...
          try {
            ...
            console.error(`Firebase uploadBytes error (USDZ ${variantName}.usdz):`, uploadErr);
            alert(`Failed to upload USDZ file for ${variantName}: ` + uploadErr.message);
            setLoadingUpload(false);
            return;
          }
          ...
          try {
            ...
            console.error(`Firebase getDownloadURL error (USDZ ${variantName}.usdz):`, urlErr);
            alert(`Failed to get download URL for USDZ file ${variantName}: ` + urlErr.message);
            setLoadingUpload(false);
            return;
          }
          usdzUrls.push(usdzDownloadUrl);
      ...
      try {
        ...
        alert("Upload succeeded, but could not copy link to clipboard. Please copy it manually: " + shareUrl);
      }
    } catch (error) {
  ...
  return (
    ...
    >
      ...
      <div
        className={`app-container ${showPreview ? "preview-mode" : ""}`}
